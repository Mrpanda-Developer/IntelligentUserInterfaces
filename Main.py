...
What it does, in order:
  1. Tries to connect to the 'BCI_FREQ' LSL stream (from
      BrainWavesVisualizer.py or BrainWavesEmulator.py).
...
class BCIListener:
    ...
    Leaves the stream unavailable so the game can use keyboard fallback if no
    LSL stream is found within 5 seconds.
    """
    def __init__(self, name="BCI_FREQ", stype="BCI", timeout=5.0):
        ...
        if self.inlet is None:
            print(f"[BCI] No LSL stream found after {timeout:.1f}s. Keyboard fallback is active.")
...
def main():
    ...
    key_to_dir = {
        pg.K_UP: "N", pg.K_w: "N",
        pg.K_LEFT: "W", pg.K_a: "W",
        pg.K_DOWN: "S", pg.K_s: "S",
        pg.K_RIGHT: "E", pg.K_d: "E",
    }
