...
3) **Maze game** → `Main.py` subscribes to `BCI_FREQ`, snaps to the nearest target frequency and moves in the corresponding direction. It also supports direct keyboard control as a fallback.
...
- Subscribes to **`BCI_FREQ`** (type `BCI`, 1 channel float32). If no stream is found shortly after startup, the game starts in keyboard mode instead.
- Press **`Tab`** to switch between **BCI** and **Keyboard** control.
- In keyboard mode, use **arrow keys** or **W/A/S/D** to move.
